...
function PropertyCard({ property }) {
  const labelClass = property.id === 3 ? 'bg-[var(--color-tertiary)]' : 'bg-[var(--color-primary)]'
    ...
    <article className="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-slate-200">
      ...
      <div className="space-y-2 p-4">
        <div className="flex items-start justify-between gap-2">
          <h3 className="text-2xl font-semibold text-[var(--color-ink)]">{property.title}</h3>
          <span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-semibold text-[var(--color-secondary)]">
            {property.rating}
          </span>
        </div>
        <p className="text-sm text-[var(--color-secondary)]">{property.location}</p>

        <div className="flex items-end justify-between pt-2">
          <p>
            <span className="text-3xl font-bold text-[var(--color-primary)]">{property.price}</span>
            <span className="text-sm text-[var(--color-secondary)]"> /month</span>
          </p>
          <button className="rounded-full p-2 text-[var(--color-primary)] transition hover:bg-blue-50">
...
function FeaturedSection() {
  return (
    <section className="bg-[var(--color-neutral)] py-16 md:py-24">
