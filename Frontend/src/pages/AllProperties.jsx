...
function FilterCheck({ label, checked = false, onToggle }) {
  return (
    <button type="button" onClick={onToggle} className="flex w-full items-center gap-2 text-left text-sm text-(--color-secondary)">
      <span className={checked ? 'inline-flex h-4 w-4 items-center justify-center rounded-sm border border-(--color-primary) bg-(--color-primary) text-white' : 'inline-flex h-4 w-4 items-center justify-center rounded-sm border border-slate-300 bg-transparent'}>
        {checked ? (
...
function FilterSidebar() {
  ...
  return (
    <aside className="hidden border-r border-slate-200 bg-[#e9edf2] p-4 lg:block">
      <div className="mb-5">
        <p className="text-xl font-bold text-(--color-primary)">Management</p>
        <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-(--color-secondary)">Premium Suite</p>
      </div>

      <div className="space-y-1 border-b border-slate-200 pb-3">
        <button type="button" className="flex h-11 w-full items-center gap-2 rounded-xl bg-white px-3 text-sm font-semibold text-(--color-primary)">
          <GridIcon />
          Overview
        </button>
        <button type="button" className="flex h-11 w-full items-center gap-2 rounded-xl px-3 text-sm font-medium text-(--color-secondary)">
          <BuildingIcon />
          Properties
        </button>
      </div>
      ...
      <div className="mt-4 border-b border-slate-200 pb-4">
        <p className="text-xs font-semibold uppercase tracking-[0.12em] text-(--color-secondary)">Price Range</p>
        <div className="mt-3 px-1">
          <div className="relative h-5">
            <div className="absolute left-0 right-0 top-1/2 h-1 -translate-y-1/2 rounded-full bg-slate-300" />
            <div className="absolute left-0 top-1/2 h-1 -translate-y-1/2 rounded-full bg-(--color-primary)" style={{ width: `${progress}%` }} />
            <div className="pointer-events-none absolute top-1/2 h-3.5 w-3.5 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white bg-(--color-primary) shadow" style={{ left: `${progress}%` }} />
            <input
              type="range"
              min={minPrice}
              max={maxPrice}
        ...
        <div className="mt-2 flex items-center justify-between text-xs font-semibold text-(--color-secondary)">
          <span>$1,500</span>
          <span>$15,000+</span>
        </div>
      </div>
      ...
      <div className="mt-4 border-b border-slate-200 pb-4">
        <p className="text-xs font-semibold uppercase tracking-[0.12em] text-(--color-secondary)">Property Type</p>
      ...
      <div className="mt-4">
        <p className="text-xs font-semibold uppercase tracking-[0.12em] text-(--color-secondary)">Amenities</p>
        <div className="mt-2 space-y-1.5">
  ...
  return (
    <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="relative h-44">
        ...
        {card.tag ? (
          <span className="absolute left-3 top-3 rounded-full bg-white/90 px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.1em] text-(--color-secondary)">
            {card.tag}
          </span>
        ) : null}
        <button
      ...
      <div className="p-3.5">
        <div className="flex items-start justify-between gap-2">
          <h3 className="text-2xl font-semibold leading-tight text-(--color-ink)">{card.title}</h3>
          <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-1 text-xs font-semibold text-(--color-secondary)">
            <StarIcon />
            {card.rating}
          </span>
        </div>
        <p className="mt-1 flex items-center gap-1 text-sm text-(--color-secondary)">
          <MapPinIcon />
          {card.location}
        </p>
        ...
        <div className="mt-4 flex items-end justify-between">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-(--color-secondary)">Monthly Rent</p>
            <p className="text-4xl font-bold text-(--color-primary)">{card.rent}</p>
          </div>
          <Link to="/properties/obsidian-penthouse" className="rounded-lg bg-[#e9eff6] px-4 py-2 text-sm font-semibold text-(--color-secondary)">
            View Details
          </Link>
        </div>
      </div>
      ...
      <div className="grid min-h-[calc(100vh-64px)] lg:grid-cols-[240px_1fr]">
        ...
        <main className="px-6 py-6">
          <div className="mb-5 flex items-end justify-between gap-4">
            <div>
              <h1 className="text-4xl font-bold text-(--color-ink)">Curated Collections</h1>
            ...
            <div className="rounded-full bg-[#dde5ee] p-1 text-sm font-semibold">
              <button className="rounded-full bg-white px-4 py-1.5 text-(--color-primary)">Grid</button>
              <button className="rounded-full px-4 py-1.5 text-(--color-secondary)">Map View</button>
            </div>
          </div>
          ...
          <footer className="mt-14 flex items-center justify-between text-xs text-(--color-secondary)">
          ...
          <button
            type="button"
            className="fixed bottom-6 right-6 inline-flex h-12 w-12 items-center justify-center rounded-full bg-(--color-primary) text-white shadow-lg shadow-blue-700/25"
          >
